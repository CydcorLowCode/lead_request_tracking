...
BEGIN
  ...
  SELECT to_jsonb(sub) FROM (
    SELECT 
      ...
      -- Multi-Users: transform numeric to Yes/No
      CASE 
        WHEN fp.periodic_usage_number::numeric > 1 THEN 'Yes'
        WHEN fp.periodic_usage_number IS NOT NULL THEN 'No'
        ELSE NULL
      END AS product_periodic_usage_number,
