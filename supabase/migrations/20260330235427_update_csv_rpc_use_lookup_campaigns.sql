...
BEGIN
  ...
  SELECT to_jsonb(sub) FROM (
    SELECT 
      ...
      -- Product fields (first linked product)
      ...
      -- Derived: Campaign Unified Code from lookup_campaigns
      -- For amazon_business: if ICL country is Canada, use amazon_business_canada code
      COALESCE(
        CASE 
          WHEN p_campaign = 'amazon_business' AND icl.country_text = 'Canada'
            THEN (SELECT lc.campaign_unified_code FROM lookup_campaigns lc WHERE lc.value = 'amazon_business_canada')
          ELSE (SELECT lc.campaign_unified_code FROM lookup_campaigns lc WHERE lc.value = p_campaign)
        END,
        'UNKNOWN'
